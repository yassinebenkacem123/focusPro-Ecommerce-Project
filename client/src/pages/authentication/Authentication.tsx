...
const Authentication = () => {
  ...
  const { username, roles, initialized, loading } = useSelector((state: RootState) => state.auth);
  ...
  useEffect(() => {
    ...
    if (oauth2 === "success") {
      // Cookie was set by backend; fetch user then redirect by role.
      void dispatch(fetchCurrentUser())
        ...
        .then((user) => {
          const userRoles = Array.isArray(user?.roles)
            ...
            : [];
          navigate(getPostAuthRedirectPath(userRoles), { replace: true });
        })
        .catch(() => {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setGoogleError(typeof message === "string" ? message : "Failed to start Google sign-in.");
    } finally {
  ...
  if (initialized && !loading && username) {
    return <Navigate to={getPostAuthRedirectPath(roles)} replace />;
  }
  ...
  return (
    ...
    <section className="relative  bg-stone-900 flex items-center justify-center h-screen w-full overflow-hidden">
      ...
      <div className="relative w-140 mx-4 p-8  flex flex-col">
        ...
        {hideSocial ? null : (
          <>
            {googleError ? <ErrorMessage message={googleError} className="mb-3" /> : null}
            ...
            <div className="flex mb-3 justify-center">
              ...
              >
                {isGoogleLoading ? (
                  <Loader message="Redirecting..." className="w-full border-0 bg-transparent p-0 text-yellow-50/80" />
                ) : (
                  <>
                    <img
